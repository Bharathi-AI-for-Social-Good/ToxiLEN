...
def process_file(input_json_path, labels_csv_path, output_json_path, model="gpt-4.1-mini-2025-04-14"):
    ...
    items = [{
        "images_name": item.get("images_name", ""),
        "text": item.get("text", ""),
        "label": 0 if str(item.get("offensiveness detection", "")).strip() == "0(non-offensive)" else 1,
    } for item in items]
    ...
    for idx, item in enumerate(items):
        ...
        # 检查 text 是否有效
        valid_text = isinstance(text, str) and text.strip() and not (isinstance(text, float) and math.isnan(text))
        if valid_text:
...
process_file(
    "data/MET/C_text.csv",
    "data/MET/label_C.csv",
    "data/MET/C_result.json",
